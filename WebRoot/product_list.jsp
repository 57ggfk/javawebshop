...
<body>
	...
	<div class="row" style="width: 1210px; margin: 0 auto;">
		...
		<c:forEach items="${requestScope.products}" var="p">
			<div class="col-md-2" style="height: 260px">
				...
				<p>
					<a href="product?pid=${p.pid }" style='color: green'>${fn:substring(p.pname,0,20) }</a>
				</p>
				<p>
	...
	<div style="width: 380px; margin: 0 auto; margin-top: 50px;">
		<ul class="pagination" style="text-align: center; margin-top: 10px;">
			...
			<c:if test="${pageBean.pageNumber>1 }">
				<li><a href="?cid=${param.cid }&page=${pageBean.pageNumber-1}"
					aria-label="Previous"><span aria-hidden="true">&laquo;</span></a></li>
			</c:if>


			<c:forEach var="index" begin="1" end="${pageBean.totalPage}" step="1"
				varStatus="state">
				<c:if test="${pageBean.pageNumber==index}">
					<li class="active"><a>${index }</a></li>
				</c:if>
				<c:if test="${pageBean.pageNumber!=index}">
					<li><a href="?cid=${param.cid }&page=${index }">${index }</a></li>
				</c:if>
			</c:forEach>
